...
% for i = 1:4
    ...
    tucker = [qOrth{1}(:, i), qOrth{2}(:, i), qOrth{3}(:, i), qOrth{4}(:, i)];
    tuckerMean = mean(mixture, 1);
    ...
    load (strcat(path,'sparseResultsSynth.mat'));
...
%    legend('forward', 'orthogonal', 'overlapped', 'mixture', 'low-rank')
    ...
    legend('Forward', 'Orthogonal', 'Convex', 'Mixture', 'Trace',  'MTL-L1', 'MTL-L21', 'MTL-Dirty','CMTL')
    xlabel('# of samples')
