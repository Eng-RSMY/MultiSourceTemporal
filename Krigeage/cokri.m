function [x0s,s,sv,idout,l,k,k0]=cokri(x,x0,model,c,itype,avg,block,nd,ival,nk,rad,ntok,b)
%
...
%
% Syntax:
% [x0s,s,sv,id,l]=cokri(x,x0,model,c,itype,avg,block,nd,ival,nk,rad,ntok,b)
%
% Input description:
...
%   b: The (rp x p) b paramter matrix of the coregionalization model.
%      Position (i,j) in each submatrix of size p x p give the b parameter 
%      of the elementary component for each cross-variogram (variogram) 
...
[x0s,s,id,l,k,k0]=cokri2(t,grid,[],model,c,sv,99,avg,ng,b);
...
for i=1:ntok:m,
   nnx=min(m-i+1,ntok);
   if (i==1 | i>=iii*ntok*100),
%     disp (['kriging points #',num2str(i),' to ', num2str(i+nnx-1) ' over ' num2str(m) ' (' num2str(round(i*1000/m)/10) '%)'])
     iii = iii+1;  
   end
   ...
   if ival>=1,
      ...
      for ip=1:np:p,
         ...
         [x0ss,ss,idout,l,k,k0]=cokri2(t,t2,id,model,c,sv,itype,avg,ng,b);
